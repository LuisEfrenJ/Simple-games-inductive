...
'''
Explicacion
1.Eligir aleatoriamente una palabra
...
letra: si no es una unica letra
indicarlo. Si ya se ha dicho indicarlo.
5.Comprobar si esa letra esta 
contenida en la palabra elegida.
6.Si está: Volver a mostrar el dibujito
de la horca como la ultima vez.
Sustituir el guión correspondiente por la letra dicha.
7.Si no está: Mostar el dibujito de la horca al que se añade una parte. Ganó¡
8.Si se falla 6 veces: se completa el dibujo del ahorcado.
9.Si se aciertan todas las letras de la palabra:
'''
import random
import os

palabras=["COLOMBIA","ECUADOR","VENEZUELA","BRAZIL","ARGENTINA","PERU","CHILE","BOLIVIA","PARAGUAY","URUGUAY","GUYANA","SURINAM"]
...
while True:
  ...
  if fallos>5:
      ...
      print("*** Has perdido ***")
      break  
  # Bucle ´para que el el usuario teclee una letra que cumpla los requisitos
  while True:
